...
export default function BubblePage() {
  ...
  const [BRIGHT, setBRIGHT] = useState<any[]>([]);
  const [NEBULA, setNEBULA] = useState<any[]>([]);
  const [CLOUDS, setCLOUDS] = useState<any[]>([]);
  useEffect(() => { setMICRO(Array.from({ length: 90 }, (_, i) => ({ id:i,   left:(Math.sin(i*7.391)*0.5+0.5)*100, top:(Math.sin(i*3.714+1.2)*0.5+0.5)*90, size:0.5+(i%3)*0.18, op:0.10+(i%8)*0.05,  dur:4+(i%7)*0.7,  delay:(i%17)*0.31 }))); }, []);
  useEffect(() => { setNORMAL(Array.from({ length: 50 }, (_, i) => ({ id:100+i, left:(Math.sin(i*5.123+0.5)*0.5+0.5)*100, top:(Math.sin(i*2.841+2.8)*0.5+0.5)*88, size:1+(i%4)*0.32, op:0.32+(i%6)*0.09, dur:2.8+(i%9)*0.42, delay:(i%11)*0.42 }))); }, []);
  useEffect(() => { setBRIGHT(Array.from({ length: 18 }, (_, i) => ({ id:155+i, left:(Math.sin(i*9.432+1.8)*0.5+0.5)*100, top:(Math.sin(i*4.567+0.3)*0.5+0.5)*80, size:1.9+(i%4)*0.38, op:0.6+(i%4)*0.08,  dur:2.2+(i%8)*0.35, delay:(i%13)*0.52 }))); }, []);
  useEffect(() => { setNEBULA(Array.from({ length: 6  }, (_, i) => ({ id:i, left:(Math.sin(i*4.123+0.7)*0.5+0.5)*100, top:(Math.sin(i*2.987+1.4)*0.5+0.5)*85, size:90+(i%4)*62, color:["rgba(70,30,160,0.055)","rgba(30,18,110,0.045)","rgba(90,50,190,0.06)","rgba(18,25,100,0.05)","rgba(50,18,130,0.045)","rgba(70,50,190,0.065)"][i] }))); }, []);
  useEffect(() => { setCLOUDS(Array.from({ length: 7  }, (_, i) => ({ id:i, left:(Math.sin(i*6.28+0.4)*0.5+0.5)*100, top:20+(Math.sin(i*3.14+1.1)*0.5+0.5)*55, w:100+(i%4)*55, h:26+(i%3)*12 }))); }, []);
  ...
  useEffect(() => {
    ...
    function spawnWave() {
      MOCK_DATA.forEach((item, i) => {
        const t = setTimeout(() => {
          addBubbleRef.current(item.text, false, item.avatar);
        }, DELAYS[i]);
        timers.push(t);
      });
    }
    ...
    spawnWave();
    const interval = setInterval(spawnWave, LOOP_INTERVAL);
  ...
  async function handleSend() {
    ...
    //     user_id: user.id, content: text, hashtags: [],
    //     color: null, session_id: null, is_offline_created: false,
    //     expires_at: new Date(Date.now() + 20 * 1000).toISOString(),
    //     lat: profileGps?.lat ?? null, lng: profileGps?.lng ?? null, radius: 50,
    //   });
